...
#..means go up a folder and then folder names to go down.
...
# echo will make the document without showing the code. if you do True, it will show all the code.
t.test(x)
```
...
#what missing data is represented as...so let the program know "na, NA, etc"
?readLines
?scan
```
...
write.table(mydata, file = "outputfile.txt", 
        ...
        #this can default to "true", so switch it to false.

write.csv(mydata, file = "outputfile.csv", row.names = FALSE) # good for writing CSVs
```
<!-- #one hash- many hashes defines differnt types of headers -->
# Data types
   - numeric
   - logical
   - character
...
#computers can only so much precision of a number. 

x <- 0.1 + 0.2
x
x == 0.3 #means that this value is EXACTLY 0.3

# "e" notation for really big numbers
1e4

# round(), floor(), ceiling()
x2 <- round(x, 1) #i rounded the value of x to 1
x2 == 0.3


# sample(), rnorm(), rbinom()

sample(10) #quick way to sample from 1 to number defined.
?sample
sample(1:10, 5) #samples without replacement
sample(1:10, 5, replace = TRUE) #samples with replacement

rnorm(10)

rbinom(10, size = 5, prob = .5)
many.flilps <- rbinom(1e6, size = 10, prob = .2)
mean(many.flilps)

?rnorm
#10 numbers from a binomail distrib, size is options (2 for coin), and prob of success (for each flip what is the prob...if weird coin that always is heads, prob is 1.0)
# the r/p/d families
...
2+2 #this is a function
'+' (2,2)

# basic forms
#they look like math but bring back logical values
1 >2 #"false"
2 >1 #"true"

class(1)
class(TRUE) #helps confirm that something is the data type you think it is

1 == 2 #this is a comparison, is EQUAL to?
x <- "a string"
x == "a string" # this is very narrow

x != "a string" # not equal to?

x <- 1:5
x <= 3
x >= 3
#will return a vector of is 1 less than or equal to 3, and so on (true and false)

TRUE & TRUE
TRUE & FALSE
# need both to be true for them to be true

TRUE | FALSE #this is or...only need 1 true
...
F
T
#just dont use them, can cause confusion
...
x == 7
which(x==7) #tells you which one comes back as your "7" values aka trues
...
x
x == c("a", "b") #nope, this iwill not help you
x %in% c("a", "b") #this will now verify with whether which values are a and b
