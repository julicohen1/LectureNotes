...
Stage 1: initial loading/fixing
  1. Read/import data
    ...
    #read.csv vs _ (they have different defaults.)
    - functions from `haven` for STATA/SPSS
    - functions from `readxl` for (fancy) Excel files
    **ask scot about masking.
    mysleep <- sleep
  2. Quick peek to check
    - head()
    - summary()
  3. Fix column names (make easier to work with)
    - colnames(yourdata) #gives you the column names
    - colnames(yourdata) <- c("vector", "of", "new", "names") #if assign a value to it you can change them.
    - colnames(yourdata)[c(1:3, 7, 14:16)] <- c("can", "rename", "just", "some", "selected", "columns", "too")
    
```{r}
mysleep <- (sleep)
colnames(mysleep)
colnames(mysleep)[c(1,3)] <- c("sleep.time", "subject") #c for a string and not a row/col
colnames(mysleep)
```
...
# spread()
#now we want a lenght column and a width column

my.iris.wideim <- spread(my.iris.long, key = "dimension", value = "cm")
head(my.iris.wideim)

ggplot(my.iris.wideim, aes(Width, Length)) + geom_point()
ggplot(my.iris.wideim, aes(Width, Length)) + geom_point() + geom_smooth(method = "lm")

ggplot(my.iris.wideim, aes(Width, Length, color = plant.part)) +
  geom_point() +
  geom_smooth(method = "lm")

ggplot(my.iris.wideim, aes(Width, Length)) +
...
ggplot(my.iris.wideim, aes(Width, Length, color = plant.part)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~ Species) #or can add scale "free"
